// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from wyrm_msgs:msg/Description.idl
// generated code does not contain a copyright notice
#include "wyrm_msgs/msg/detail/description__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `name`
#include "rosidl_runtime_c/string_functions.h"

bool
wyrm_msgs__msg__Description__init(wyrm_msgs__msg__Description * msg)
{
  if (!msg) {
    return false;
  }
  // id
  // parent_id
  // name
  if (!rosidl_runtime_c__String__init(&msg->name)) {
    wyrm_msgs__msg__Description__fini(msg);
    return false;
  }
  // num_markers
  return true;
}

void
wyrm_msgs__msg__Description__fini(wyrm_msgs__msg__Description * msg)
{
  if (!msg) {
    return;
  }
  // id
  // parent_id
  // name
  rosidl_runtime_c__String__fini(&msg->name);
  // num_markers
}

bool
wyrm_msgs__msg__Description__are_equal(const wyrm_msgs__msg__Description * lhs, const wyrm_msgs__msg__Description * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // id
  if (lhs->id != rhs->id) {
    return false;
  }
  // parent_id
  if (lhs->parent_id != rhs->parent_id) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->name), &(rhs->name)))
  {
    return false;
  }
  // num_markers
  if (lhs->num_markers != rhs->num_markers) {
    return false;
  }
  return true;
}

bool
wyrm_msgs__msg__Description__copy(
  const wyrm_msgs__msg__Description * input,
  wyrm_msgs__msg__Description * output)
{
  if (!input || !output) {
    return false;
  }
  // id
  output->id = input->id;
  // parent_id
  output->parent_id = input->parent_id;
  // name
  if (!rosidl_runtime_c__String__copy(
      &(input->name), &(output->name)))
  {
    return false;
  }
  // num_markers
  output->num_markers = input->num_markers;
  return true;
}

wyrm_msgs__msg__Description *
wyrm_msgs__msg__Description__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  wyrm_msgs__msg__Description * msg = (wyrm_msgs__msg__Description *)allocator.allocate(sizeof(wyrm_msgs__msg__Description), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(wyrm_msgs__msg__Description));
  bool success = wyrm_msgs__msg__Description__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
wyrm_msgs__msg__Description__destroy(wyrm_msgs__msg__Description * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    wyrm_msgs__msg__Description__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
wyrm_msgs__msg__Description__Sequence__init(wyrm_msgs__msg__Description__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  wyrm_msgs__msg__Description * data = NULL;

  if (size) {
    data = (wyrm_msgs__msg__Description *)allocator.zero_allocate(size, sizeof(wyrm_msgs__msg__Description), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = wyrm_msgs__msg__Description__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        wyrm_msgs__msg__Description__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
wyrm_msgs__msg__Description__Sequence__fini(wyrm_msgs__msg__Description__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      wyrm_msgs__msg__Description__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

wyrm_msgs__msg__Description__Sequence *
wyrm_msgs__msg__Description__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  wyrm_msgs__msg__Description__Sequence * array = (wyrm_msgs__msg__Description__Sequence *)allocator.allocate(sizeof(wyrm_msgs__msg__Description__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = wyrm_msgs__msg__Description__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
wyrm_msgs__msg__Description__Sequence__destroy(wyrm_msgs__msg__Description__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    wyrm_msgs__msg__Description__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
wyrm_msgs__msg__Description__Sequence__are_equal(const wyrm_msgs__msg__Description__Sequence * lhs, const wyrm_msgs__msg__Description__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!wyrm_msgs__msg__Description__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
wyrm_msgs__msg__Description__Sequence__copy(
  const wyrm_msgs__msg__Description__Sequence * input,
  wyrm_msgs__msg__Description__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(wyrm_msgs__msg__Description);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    wyrm_msgs__msg__Description * data =
      (wyrm_msgs__msg__Description *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!wyrm_msgs__msg__Description__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          wyrm_msgs__msg__Description__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!wyrm_msgs__msg__Description__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
